...
describe.skipIf(!testDb)("profile (integration)", () => {
  ...
  beforeAll(async () => {
    ...
    const user = await db.user.create({
      data: { username, normalizedUsername: username, passwordHash: "x" },
    });
    userId = user.id;
    const stranger = await db.user.create({
      data: { username: `${prefix}_stranger`, normalizedUsername: `${prefix}_stranger`, passwordHash: "x" },
    });
  ...
  it("exposes only public-safe fields", async () => {
    ...
    const serialized = JSON.stringify(profile, (_, value) =>
