...
import { listInventory } from "./inventory-query";
...
describe.skipIf(!testDb)("listInventory (integration)", () => {
  ...
  beforeAll(async () => {
    const user = await db.user.create({
      data: { username: `${prefix}_user`, normalizedUsername: `${prefix}_user`, passwordHash: "x" },
    });
    userId = user.id;
  ...
  it("sorts by value descending", async () => {
    ...
    expect(entries.map((entry) => entry.item.price)).toEqual([40n, 25n, 10n]);
  });
