...
export async function getFortuneView(
  ...
  // No list of recent pulls. One was built here and never rendered, and a
  // query nobody reads is a cost with no reader — the machine shows the
  // pull you just made, which is the one a player is looking at.
  const [jackpot, user, best] = await Promise.all([
